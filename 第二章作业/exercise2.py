...
def newton_interpolation(x_list,y_list):
    ...
    return expression
def linear_interpolation(x_list,y_list):
    def mult(n,k=1):
        if(n>k or n==1):
            return 1
        else:
            return k*mult(n,k+1)
    def difference(x_list,y_list,i,step):
        if i==1:
            return y_list[0]
        else:
            return mult(i)*difference_quotient(x_list[0:i],y_list[0:i],i)*(step**i)
    def var(i,k=0):
        if(k>i):
            return 1
        else:
            return (t-k)*var(i,k+1)
    def main(x_list,y_list):
        assert len(x_list)==len(y_list)
        step=x_list[1]-x_list[0]
        degree=len(x_list)
        var_list,param_list=[],[]
        for i in range(1,degree+1):
            var_list.append(difference(x_list,y_list,i))
            param_list.append(var(i)/mult(i))
        return sum(list(map(lambda x,y:x*y,var_list,param_list)))
